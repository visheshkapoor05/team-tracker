...
export function HolidayManager({
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5">
      ...
      <div className="mb-5 flex flex-wrap items-end gap-3 rounded-lg bg-slate-50 p-3">
      ...
      <div className="overflow-x-auto rounded-lg border border-slate-100">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={`${row.date}|${row.label}`} className="border-t border-slate-100">
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={2 + offices.length} className="px-3 py-6 text-center text-slate-400">
