...
export function TrackerViewSelector({
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute left-0 z-30 mt-1 w-56 overflow-hidden rounded-md border border-slate-200 bg-white py-1 shadow-lg">
          <button
            onClick={() => {
            ...
            className={`block w-full px-3 py-2 text-left text-sm hover:bg-slate-50 ${
              viewingSelf ? "bg-indigo-50 font-medium text-indigo-700" : "text-slate-700"
            }`}
          >
            My tracker
          </button>
          ...
          {others.map((p) => (
            <button
              ...
              className={`block w-full px-3 py-2 text-left text-sm hover:bg-slate-50 ${
                p.id === viewingProfileId ? "bg-indigo-50 font-medium text-indigo-700" : "text-slate-700"
              }`}
            >
              {p.full_name}
              <span className="ml-1.5 text-xs font-normal text-slate-400">({p.role})</span>
            </button>
          ))}
        </div>
      )}
